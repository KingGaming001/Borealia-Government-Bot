...
def has_king_role(member: discord.Member, settings: dict | None) -> bool:
    """
    True if member has the configured king role.
    """
    if not settings:
        return False
    king_role_id = settings.get("king_role_id")
    if not king_role_id:
        return False
    return any(r.id == int(king_role_id) for r in member.roles)


def has_cabinet_role(member: discord.Member, settings: dict | None) -> bool:
    """
    True if member has the configured cabinet role.
    """
    if not settings:
        return False
    cabinet_role_id = settings.get("cabinet_role_id")
